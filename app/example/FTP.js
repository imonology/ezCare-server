...
l_handlers.COMP_GETPAGE = function (event) {
	...
	var cb_getPage = function (err_getPage, docs, last_doc) {
		if (err_getPage) {
			...
			IC.DB.getPageBySkippingDocs(IC.Settings.DB_NAME_SYS_EVENT, q, opts, page_num, cb_getPageBySkippingDocs);
		}
	};
...
l_handlers.SYS_EVENT_COUNT = function (event) {
	...
	IC.DB.count(IC.Settings.DB_NAME_SYS_EVENT, onSuccess);
};
...
l_handlers.ADD_LOG_EVENT = function (event) {
	...
	while (count < event.data.n) {
		count++;
		LOG.event("STRESS TEST", IC.Settings.SERVER_INFO);
	}
	event.done();
};
...
l_handlers.TEST_LOG = function (event) {
    ...
    IC.DB.getArray(IC.Settings.DB_NAME_SYS_EVENT, onSuccess, onFail, q, cond);
};


l_handlers.TEST_CTM = function (event) {
	IC.Notify.customizeMethod("Hello", function (info, user) {
...
l_handlers.TEST_REG = function (event) {
	...
	IC.Notify.register(event.data.level, [/*"SMS", */"email", "client", "Hello"], event.session._account, cb);
	event.done();
};
...
l_handlers.TEST_REGALL = function (event) {
	...
	IC.Notify.register(event.data.level, ["email", "client", "Hello"], cb);
	event.done();
};
...
l_handlers.TEST_CONN = function (event) {
	...
	LOG.warn(event.conn);
	IC.Notify.subscribe(event.session._account, event.conn, cb);
	event.done();
};

l_handlers.TEST_ALERT = function (event) {
	IC.Notify.alert(event.data.type, {msg: event.data.msg}, event.data.level);
	event.done();
};
