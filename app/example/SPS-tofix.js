...
var l_onPosition = function (update) {
    ...
    if (l_nodes.hasOwnProperty(update.to)) {
        ...
        IC.EventManager.send('NODE_UPDATE', {ident: update.ident, pos: update.pos}, [node.conn.connID]);
    }
}
...
l_HandlerPool.QUERY_SERVER = function (event) {
	...
	var IPport = IC.AppHandler.queryAppServer(name);
...
l_HandlerPool.CREATE_NODE = function (event) {
    ...
    IC.Comm.createNode(para.id, para.layer, para.pos, l_onPosition, l_onMessage, function (node) {
		...
		IC.Conn.setConnName(event.conn.connID, node.ident);
        
        // save data 
        var create_time = new Date().toISOString();
        IC.DB.setData('users', {name: node.ident, time: create_time}, 
            function (msg) {
...
l_HandlerPool.DELETE_NODE = function (event) {
	...
	var ident = IC.Conn.getConnName(event.conn.connID); 
...
IC.Callback.onDisconnect(function (conn) {
    ...
    // look for ident
    var ident = IC.Conn.getConnName(conn.connID);
