...
//var l_users = IC.State.get('users', 'map'); // indexed by account
...
l_handlers.USER_LOGIN = function (event) {
	...
	if (username === undefined)
		username = 'user ' + IC.Utility.createID();
	
	LOG.warn('username: ' + username);
	
	
	// store user name
    IC.Conn.setConnName(event.conn.connID, username);
    ...
    if (pending.length > 0) {                
        ...
        IC.EventManager.send('GAME_START', {opponent: conn2.name, msg: msg}, [conn1]);
        IC.EventManager.send('GAME_START', {opponent: conn1.name, msg: msg}, [conn2]);
...
IC.Callback.onDisconnect(function (conn) {
    ...
    if (started.hasOwnProperty(connID)) {
        ...
        LOG.warn(msg);
        IC.EventManager.send('GAME_OVER', {depart: username, msg: msg}, [game.remote_conn]);
    }
