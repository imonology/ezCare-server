...
l_handlers.TEST_CLUSTER = function (event) {
	...
	// send back response
	event.done('TEST_CLUSTER', {server: IC.Settings.SERVER_INFO, name: event.data.name, age: age, 中文: 'Chinese 也通!', reset_counter: reset_counter, persist_counter: l_states['counter']});
}
...
l_handlers.QUERY_APP = function (event) {
	...
	result = IC.AppConn.queryAppServers(event.data.name);
...
l_handlers.APP_EVENT = function (event) {
	...
	var obj = IC.kit.clone(event.data);
	delete obj['server'];
	delete obj['event'];
	
	IC.RPC.remoteEvent(event.data.server, event.data.event, obj, onResponse);
	// do something..
}
...
l_handlers.RELAY_EVENT = function (event) {
	...
	IC.RPC.relayEvent(server_name, event_name, event);
}
...
l_handlers.SEND_CONN = function (event) {
	...
	var list = IC.Conn.getConnections(name);
	...
	if (event.data.who && event.data.msg)
		event.send('IC_MSG', {"channel":"main","msg":{"who": event.data.who, "msg": event.data.msg}}, list, false);
}