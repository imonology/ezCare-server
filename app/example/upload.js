...
l_handlers.SHOW_UPLOAD = function (event) {
	var path = IC.Settings.FRONTIER_PATH + '/../upload/';
	
	IC.fs.readdir(path, function (err, files) {
...
l_handlers.DOWNLOAD = function (event) {
	...
	var path = IC.Settings.FRONTIER_PATH + '/../upload/' + event.data.filename;
	LOG.warn('retriving path: ' + path);
	event.done("IC_RESOURCE", {address: path, 
							   header: {}
							  });
};